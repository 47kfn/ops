...
**明確化方針**: current mode を本文の主軸とし、曖昧な判断基準・責務分担・優先順位・境界条件を明文化する
...
| 役割 | デフォルト行動 |
|---|---|
| Triage | queue 全状態を **優先順位に従って** スキャンし（stale running → blocked → review → running → proposed → ready → inbox）、TRIAGE_REPORT と CANDIDATES を生成する |
| Executor | running/ にある自分の task packet の HANDOFF.md を読み、「次の1〜3手」から再開する |
| Reviewer | review/ にある task packet を **古い順** に選び、diff と VERIFY.md を読み、REVIEW.md を書く |
...
| ready → running | **AI自動**（current mode: 人間がsession起動後にAIが実行） | worktree作成と実装は機械的 |
...
## 0-3a. Current Mode と Future Mode の区分

この設計書は **current mode（現在運用）** を本文の主軸とする。future mode は補足として記載する。

### Current Mode（本文の主軸）

| 項目 | 現在の実装 |
|---|---|
| セッション起動 | **人間が手動で** `claude` コマンドを実行する |
| ready → running | 人間がセッションを起動し、AI が worktree 作成・実装を行う |
| Triage 起動 | 人間が「triage して」と入力、または triage session を手動で起動する |
| Review 起動 | 人間が review session を手動で起動する |
| 並列実行 | 人間が複数ターミナルで session を起動することで実現 |
| 定期実行 | `/schedule` で triage を定期起動（設定後は自動） |
| proposed → ready | 人間が判断して昇格 |
| review → done | 人間が REVIEW.md + diff を見て merge 判断 |

### Future Mode（補足扱い）

| 項目 | 将来の実装候補 |
|---|---|
| セッション起動 | 永続エージェント / Agent SDK daemon による自動起動 |
| ready → running | Agent SDK が queue を監視し、自動で session 起動 |
| 定期実行 | Agent SDK cron / event handler |
| 並列実行 | Agent Team 公式サポート |

**本文中の「AI自動」の読み方**: 本文で「AI自動」と記載されている遷移のうち、「セッションが既に起動している状態でAIが自律的に実行する」ものは current mode で成立する。「セッション自体の起動」が必要なものは、current mode では人間がトリガーする。
...
**current mode で人間がトリガーするもの（「やらなくていい」ではないが軽い操作）:**
...
人間がセッションを開始した時、以下の優先順位で処理対象を決定する。これは Triage Agent のデフォルト行動にも、人間が DASHBOARD.md を見て判断する時にも適用される **設計上の優先原則** である。

```
優先順位（上から順に処理する）:

  1. stale running  — 2時間以上 HANDOFF.md 未更新の running task
     → 救済判定: blocked に移すか、session を再起動して継続するか
  2. blocked        — 人間判断待ちの停止タスク
     → 判断して ready に戻すか、保留するか
  3. review         — レビュー待ちの完了タスク
     → REVIEW.md + diff を見て merge / 差し戻し / hold を判断
  4. running (正常) — 正常に進行中のタスク
     → 確認のみ。介入不要
  5. proposed       — ready に昇格する候補
     → 今回のセッションで回す本数を決めて ready に昇格
  6. ready          — 着手可能だがまだ session 未起動のタスク
     → session を起動して実行開始
  7. inbox          — 未整理の投入物
     → Triage Agent に任せる（優先度は低い）
```

**原則: 後処理優先** — セッション開始時は「前回の実装の後処理」（stale / blocked / review）を先に処理し、新規着手（proposed → ready → running）はその後に行う。
...
## 2-3. 各スコープに持たせるもの — 責務境界の明確化

以下の原則で「何をどこに書くか」を判定する:

| 判定基準 | 書く場所 |
|---|---|
| 物理的に破ってはいけない制約 | **permissions** |
| 必ず実行されるべきライフサイクル処理 | **hooks** |
| 状況に応じた振る舞いの切り替えルール | **rules/** |
| プロジェクト全体の方針・構造・定義 | **CLAUDE.md** |
| 再利用する複数ステップの手順 | **skills/** |
| 役割ごとの権限・ツール制約 | **agents/** |

迷った場合の判定: 「強制力が必要か」→ Yes なら permissions/hooks。「状況依存か」→ Yes なら rules/。「全セッションで常に参照されるべきか」→ Yes なら CLAUDE.md。「手順を繰り返し呼ぶか」→ Yes なら skills/。
...
- ワークフローの手順
- 判断基準
- 「推奨」や「望ましい」レベルの制約（強制力が必要ないものは rules/ へ）
```
...
```
持たせるもの:
- セッション開始時の文脈チェック・役割情報の注入（UserPromptSubmit）
- 危険操作の遮断（PreToolUse）
...
- 判定ルールの本体（hooks は skill を呼ぶだけ。ロジックは skill 内に書く）
```

### rules/（条件付き注入。状況に応じて読まれる）

「この状況ではこう振る舞え」という状況依存のガイダンス。**判断ルール・振る舞い条件の定義場所**。
...
- role-detection.md: 役割自動判定ルール（判定ロジック・優先順位）
- autonomous-ops.md: 自律運転ルール（自動進行・停止・retry の条件）

持たせないもの:
- プロジェクト固有の仕様（それは 00-docs/ に書く）
- タスク固有の手順（それは task packet に書く）
- 定義や構造の説明（それは CLAUDE.md に書く）

判断基準: rules/ には「条件 → 行動」の形式で書く。
「Xの定義は何か」は CLAUDE.md、「Xの時どうするか」は rules/。
```

### CLAUDE.md（context。方針と構造の説明）

「このプロジェクトは何で、どういう構造で、何が大事か」の全体像。**定義と構造の宣言場所**。
...
- Queue State Machine の状態定義（状態の意味と遷移先）
- Task Packet の必須ファイル定義
- Done / Review の定義
- 役割判定の基本ルール（cwd + queue + permission → 役割）
- デフォルト行動の定義（各役割が「何も指示されなかった場合」にやること）
- 状態処理優先順位の定義
...
- 条件分岐を伴う判断ロジック（それは rules/ に書く）

判断基準: CLAUDE.md には「何がXか」「Xとは何か」を書く。
「Xの時どうするか」は rules/、「Xを実行する手順」は skills/。
```

### skills/（呼ばれた時だけ実行される再利用手順）

「この手順をパッケージとして呼び出す」。**実行手順の実体**。

```
持たせるもの:
- triage-from-state: 状態スキャンと候補生成
- make-task-packet: task packet 雛形生成
- failure-classifier: 失敗分類（判定ロジックの実体はここ）
...
持たせないもの:
- 1回限りのアドホック手順
- 判断基準の宣言（それは rules/ に書く。skill は rules/ の基準に従って実行する）
- 定義の説明（それは CLAUDE.md に書く）
```

### agents/（役割分離と context 保護）

「この役割にはこの権限とツールだけ」。**権限スコープの制約場所**。
...
### Current mode で短プロンプトが成立する範囲と、明示的操作がまだ必要な範囲

| 操作 | 短プロンプトで成立するか | 理由 |
|---|---|---|
| 「triage して」 | **成立する** | CLAUDE.md + rules/ + skills/ で役割・手順が自動決定される |
| 「review して」 | **成立する** | review/ の存在 + role-detection で Reviewer として動作する |
| 「Q-P1-0042 を実行して」 | **成立する** | task.yaml + HANDOFF.md から自動再開する |
| 「状況を教えて」 | **成立する** | dashboard skill が queue 状態を読んで返す |
| ready にある task の実行開始 | **成立しない** | 人間が worktree に cd して session を起動する必要がある |
| 新 session の起動自体 | **成立しない** | 人間が `claude` コマンドを実行する必要がある |
| proposed → ready の昇格 | **成立しない** | 人間の経営判断が必要（設計上の意図的なゲート） |
| review → done（merge）| **成立しない** | 人間の最終承認が必要（設計上の意図的なゲート） |

**構造で成立する条件**: CLAUDE.md に定義があり、rules/ に判定ロジックがあり、skills/ に手順があり、hooks が文脈を注入している場合、プロンプトは「やって」だけで足りる。

**明示操作が必要な条件**: セッション起動自体、人間ゲートの通過、permission mode の切り替えは、current mode では人間が操作する。
...
2. cwd がメインrepo か？
   YES → メインrepoセッション
     → 以下の優先順位で役割を判定する:
       ...
       優先度1: プロンプトに明示的な役割指示がある場合はそれに従う
         - 「review して」→ Reviewer
         - 「triage して」→ Triage

       優先度2: プロンプトに明示指示がない場合、queue 状態から判定する
         - review/ にタスクがある → Reviewer を優先する
           （理由: 後処理優先の原則。新規着手より完了判定を先に行う）
         - review/ が空で、stale running または blocked がある → Triage
           （理由: 異常状態の整理を優先する）
         - 上記のいずれでもない → Triage（デフォルト行動: 状態スキャン）
...
**Triage と Review の優先ルール（current mode）**: メインrepo + plan mode のセッションで review/ にタスクがある場合、明示指示がなければ **Review を優先する** 。Triage は review 消化後、または明示指示で行う。これは「後処理優先」の設計原則に基づく。
...
処理（7-1a. の優先順位に従ってスキャン）:
  1. 異常状態を先にスキャンする（後処理優先）
     - stale running: HANDOFF.md が 2時間以上未更新の running task（閾値は parallel.yaml で設定）
     - blocked/: 停滞タスクの有無と原因
     - review/: レビュー待ちタスクの有無と件数
  2. 正常状態をスキャンする
     - running/: 正常に進行中のタスクの進捗
     - proposed/: 既存候補の状態
     - ready/: 着手可能タスクの有無
     - inbox/: 新規投入物の有無
     - done/: 直近完了タスク
  3. 異常を集約する
     - stale running の一覧と推奨アクション
     - 同種 blocked の蓄積（3件以上）→ infra-fix task の提案
     - orphan worktree（running task がないのに worktree が残っている）
     - review backlog 警告（3件以上で新規着手抑制を推奨）
  4. TRIAGE_REPORT と CANDIDATES を生成する
     - CANDIDATES には afk_safe フラグを含める
  5. 人間向けサマリーを出力する
```
...
### ready → running（current mode: 人間トリガー + AI実行）

```
current mode:
  トリガー: 人間が worktree に cd して session を起動する、
            または Triage session 内で「Q-xxx を実行して」と指示する
  処理:
    1. worktree-execution skill を実行（AI が自動で worktree 作成・task packet 移動）
    2. task packet を running/ に移動
    3. task.yaml の status を "running" に更新
    4. Executor セッションを起動（起動コマンドを人間に提示、または worktree 内の場合は自動開始）

future mode:
  トリガー: Agent SDK daemon が ready/ を監視し、並列度上限に達していなければ自動起動
```

**current mode での注意**: ready → running の「自動」は「session が既に起動している状態で AI が worktree 作成から実行までを自律的に行う」ことを意味する。session 自体の起動は人間がトリガーする。
...
### review backlog の処理方針

**原則: review 消化は新規着手に優先する**。

```
条件と行動:
- review/ が 3件以上 → 新規 ready → running を抑制し、review 消化を優先する
- review/ が 5件以上 → 新規着手を完全停止し、全セッションで review 消化に集中する
- review/ が 1-2件 → 通常運用。ただしセッション開始時は review を先に処理する（1-4. 優先順位に従う）

判断者:
- current mode: 人間が DASHBOARD.md を見て判断する。Triage Agent は TRIAGE_REPORT で review backlog 警告を出す
- future mode: auto-advance skill が自動判定する
```

### AFK 中に回してよいタスクの基準

人間が離席（AFK: Away From Keyboard）中に AI に自律実行させるタスクは、以下の基準を全て満たすものに限る。Triage Agent は triage 時にこの基準で「AFK 実行可能」フラグを task.yaml に付与する。

```
AFK 実行可能の条件（全て満たす）:
  1. blocked 時に人間判断が不要な見込みが高い
     - target_paths が明確で、forbidden_paths に抵触しない
     - spec_ambiguity リスクが低い（confidence ≥ 7）
  2. 失敗しても安全に停止できる
     - 破壊的操作を含まない
     - 課金リスクがない
     - 環境を壊さない（compose down -v 等が不要）
  3. タスク粒度が適切
     - 想定実行時間 2時間以内
     - definition_of_done が 5個以下
  4. 依存関係がない
     - related_tasks に running 中のタスクがない

task.yaml への記載:
  afk_safe: true/false    # Triage Agent が判定
  afk_reason: "..."       # false の場合、理由を記載
```
...
### stale running の処理（設計方針）

**設計思想**: stale running は「session が死んだ可能性がある異常状態」であり、新規タスクの着手より先に救済・整理すべき対象である。

```
閾値: HANDOFF.md が 2時間以上未更新

判定優先順位:
  1. stale running の検出と救済は、新規タスクの ready → running より優先する
  2. stale running は Triage session 開始時の最初のチェック項目に含まれる
  3. 複数の stale running がある場合は、HANDOFF.md の進捗が深い（完了条件の達成率が高い）ものを優先する
...
処理:
  ...
  2. 生きていない場合:
     ...
     - 分類結果に応じて:
       a. HANDOFF.md に明確な「次の1〜3手」がある → session 再起動で継続（ready に戻さず running のまま）
       b. HANDOFF.md が古すぎるか不明確 → blocked に移動
       c. 明らかな環境障害 → blocked + infra-fix task を proposed に追加
  3. 通知: DASHBOARD.md の警告欄に記録
```
...
## 5-3a. merge 判断のための最小確認セット

非エンジニアが review → done（merge）を判断するために、以下の材料が揃っていることを設計上の要件とする。Review Agent はこれらを全て REVIEW.md 内に集約する義務がある。

```
merge 判断に必要な最小確認セット:

  1. REVIEW.md の「判定」欄
     - approve candidate / fix before merge / blocked / reject の4択
     - Review Agent の判定理由（1-3行）

  2. REVIEW.md の「差分サマリー」欄
     - 変更ファイル数、追加行数、削除行数
     - 変更の概要（非エンジニア向け、1-3行の日本語説明）

  3. REVIEW.md の「チェック項目」テーブル
     - 全項目が ok/ng で埋まっていること
     - ng がある場合は対応方針が記載されていること

  4. VERIFY.md へのリンク（テスト結果の事実）

  5. diff の確認方法
     - REVIEW.md 内に `git diff main...{branch} --stat` の出力を含める
     - 詳細を見たい場合のコマンドを記載する
```

**判断の目安（非エンジニア向け）**:
- REVIEW.md の判定が「approve candidate」かつチェック項目が全て ok → merge してよい
- 1つでも ng がある → fix を指示するか、保留する
- 判定が「blocked」または「reject」→ merge しない

## 5-3b. blocked 時の人間向け出力要件

blocked に遷移するタスクは、非エンジニアが即座に判断できるよう、以下の形式で情報を整理する義務がある。これは failure-classifier skill と Execution Agent の出力要件として定義する。

```
blocked_reason.md の必須構成:

  ## 何が止まっているか（1行）
  例: 「Difyのcompose起動がDB接続エラーで失敗している」

  ## なぜ止まっているか（1-3行、技術用語を最小限に）
  例: 「DBの接続先設定が.envファイルに書かれているが、
       その設定を変更するにはsecretsディレクトリへのアクセスが必要で、
       これはセキュリティ上AI側で操作できない」

  ## 人間に何をしてほしいか（具体的な選択肢を提示）
  例:
  - 選択肢A: .envの接続先を手動で修正する → その後「Q-P1-0042 を再開して」で再開可能
  - 選択肢B: このタスクを保留にして別タスクを先に進める
...
分類ごとの追加要件:
  - spec_ambiguity: spec-question.md を作成し、複数の解釈を明示する
  - needs_human_decision: 選択肢を2-3個に絞り、各選択肢の影響を簡潔に説明する
  - permission: 必要な権限変更の具体的手順を記載する
  - environment: 環境の問題箇所と、人間が確認すべきポイントを記載する
```

## 5-4. 人間の操作パターン（3つだけ）

いずれのパターンでも **後処理優先の原則**（Section 1-4, 7-1a, 25-1）に従う。

### パターン A: 仕事前の確認と指示

```
1. DASHBOARD.md を開く
2. 後処理を先に片付ける（Phase 1）:
   - stale running があれば救済判定
   - blocked があれば判断（ready に戻す / 保留）
   - review があれば merge / 差し戻し判断
3. 後処理が終わってから新規着手（Phase 2）:
   - proposed の上位を ready に昇格させる
   - ready タスクの session を起動する
4. 終了
```
...
1. DASHBOARD.md を開く
2. 後処理を先に片付ける:
   - review があれば merge 判断（running slot を空ける）
   - blocked があれば判断（翌朝まで放置してよいか判定）
3. AFK 実行可能な ready タスクがあれば session 起動
4. running の進行を確認し、問題なければ放置
```
...
3. blocked_reason.md の「人間に何をしてほしいか」欄の選択肢から判断する
...
Queue 内の複数状態にタスクがある場合、以下の優先順位で処理する。これは Triage Agent、人間の判断、セッション開始時のデフォルト行動に共通する設計原則である。

| 優先順位 | 状態 | 処理内容 | 理由 |
|---|---|---|---|
| 1 | stale running | 救済判定（blocked 移動 or session 再起動） | 異常状態の放置は被害拡大 |
| 2 | blocked | 人間判断の実施 | 後続タスクの unblock に直結 |
| 3 | review | merge / 差し戻し / hold の判断 | 完了タスクの確定を先に行う |
| 4 | running（正常） | 確認のみ | 介入不要。正常進行中 |
| 5 | proposed | ready への昇格判断 | 新規着手の準備 |
| 6 | ready | session 起動による実行開始 | 新規着手 |
| 7 | inbox | Triage Agent による構造化 | 整理の優先度は低い |

**新規着手に進んでよい条件**:
- stale running が 0 件
- blocked で即判断可能なものが処理済み（保留判断も「処理済み」に含む）
- review が 3件未満（3件以上の場合は review 消化を優先）
...
| ready → running | AI（current mode: 人間がsession起動） | ready に存在 + 並列度上限未達 | worktree 作成 + session 起動 |
...
### ready → running
- **current mode**: 人間が session を起動し、AI が worktree 作成・task packet 移動・実装を自律実行
- **future mode**: AI が自動で session 起動を含めて実行
- worktree 作成
- task packet の `running/` への移動
- Claude session 起動（current mode では人間が操作）
...
- プロンプトに明示的な役割指示がある → その役割に従う
- 明示指示がなく review/ にタスクがある → **Reviewer**（後処理優先の原則）
- 明示指示がなく review/ が空 → **Triage**
...
以下のイベントで自動起動する。**current mode** と **future mode** の実装方法を明確に区別する。

| イベント | current mode の実装 | future mode の実装候補 |
|---|---|---|
| run 終了 | Stop hook → triage skill 呼び出し（session 内で自動） | Agent SDK daemon |
| blocked 発生 | PostToolUse hook → failure-classifier（session 内で自動） | Agent SDK event handler |
| review 差し戻し | SubagentStop hook → triage update（session 内で自動） | Agent SDK event handler |
| inbox に仕様変更 | 人間が次の triage session で impact-analyzer を呼ぶ | Agent SDK watcher |
| stale running 検出 | `/schedule` による定期チェック、または人間が triage session で確認 | Agent SDK cron |

**current mode の制約**: hooks によるイベント駆動は「session が起動している間」のみ有効。session が起動していない間に発生したイベント（夜間の stale running 等）は、次の session 開始時に Triage が検出する。
...
詳細な操作手順ではなく、日次運用の判断がブレないための **設計上の優先原則** を定める。

## 25-1. セッション開始時の処理原則

**後処理優先の原則**: どの時間帯でも、セッション開始時は「前回実装の後処理」から始める。新規着手はその後。

```
処理順序（7-1a. の優先順位に従う）:

  Phase 1: 後処理（まずここを片付ける）
    1. stale running の救済判定
    2. blocked の判断（即決できるものを処理、保留も判断に含む）
    3. review の merge / 差し戻し判断

  Phase 2: 新規着手（Phase 1 を終えてから）
    4. proposed → ready の昇格判断
    5. ready タスクの session 起動
...
**目的**: 翌朝まで AI が自律運転できる状態を作る。

```
優先事項:
  1. review/ の消化（merge できるものを merge し、running slot を空ける）
  2. blocked の判断（翌朝まで放置してよいか判定）
  3. AFK 実行可能（afk_safe: true）な ready タスクの session 起動
  4. running タスクの HANDOFF.md 鮮度確認

やらないこと:
  - AFK 実行不可（afk_safe: false）なタスクの新規着手
  - 仕様判断が必要な blocked の深追い（翌朝に回す）
```

## 25-3. 仕事前セッションの原則

**目的**: 夜間結果の確認と、日中の自律運転準備。

```
優先事項:
  1. DASHBOARD.md で夜間の結果確認（done / blocked / stale の発生）
  2. review/ の消化（夜間に完了したタスクの merge 判断）
  3. blocked の判断（夜間に発生したもの）
  4. proposed の昇格判断（今日回す本数を決める）
  5. ready タスクの session 起動

やらないこと:
  - 時間が足りない場合に review を飛ばして新規着手すること（後処理優先）
```

## 25-4. AI の自律運転範囲（current mode）

```
session が起動している間、AI が自動で行うこと:
  - 実装: BRIEF → PLAN → HANDOFF に従って実装
  - テスト: VERIFY.md を更新
  - review-gate 通過: running → review に自動遷移
  - 失敗時: failure-classifier で分類、retry or blocked
  - HANDOFF.md: 区切りごとに更新
  - DASHBOARD.md: 状態変化時に更新

session が起動していない場合、AI ができないこと（人間が操作）:
  - 新しい session の起動
  - worktree への cd と claude コマンド実行
  - proposed → ready の昇格
  - review → done の merge 判断
```

## 25-5. review backlog が溜まった場合の原則

```
review/ の件数に応じた行動:
  - 1-2件: 通常運用。セッション開始時に Phase 1 で処理する
  - 3-4件: 新規着手を抑制。review 消化を優先する
  - 5件以上: 新規着手を完全停止。全セッションで review 消化に集中する
```
...
## やるが、AI/ツール進化に応じて置き換わるもの（current mode → future mode）

| current mode の実装 | future mode の置き換え候補 | 置き換えの条件 |
|---|---|---|
| 手動 session 起動 | 永続エージェント / Agent SDK daemon | Agent SDK の daemon モードが安定 |
| /schedule による定期 triage | Agent SDK cron / event handler | event-driven triage が実用レベル |
| hooks による安全装置 | auto permission mode の成熟 | auto mode の安全性が実証済み |
| subagent 手動定義 | Agent Team 公式サポート | 公式の並列 agent 管理が安定 |
| gh CLI による Issue 同期 | GitHub Actions の深い統合 | Actions の Claude 統合が十分 |
| plan/acceptEdits の手動切替 | auto mode の安定化 | auto mode で deny rules が十分機能 |
...
**current mode の設計精度について**: 本設計書は current mode（人間が session を起動し、AI が構造に従って自律実行する運用）を本文の主軸とする。以下の判断基準が明文化されている:
- 状態処理の優先順位（7-1a）
- 後処理優先の原則（1-4, 25-1）
- Triage / Review の役割判定優先順（3-1）
- 新規着手に進んでよい条件（7-1a）
- AFK 中に回してよいタスクの基準（4-3）
- review backlog の処理方針（4-3）
- merge 判断の最小確認セット（5-3a）
- blocked 時の人間向け出力要件（5-3b）
- 構造で成立する範囲と人間操作が必要な範囲の境界（0-3a, 2-4）
- CLAUDE.md / rules / skills / agents / hooks の責務境界（2-3）
