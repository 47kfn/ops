...
目的はいきなり実装に入ることではない。草案を開発用ドキュメントに整理し、未決定事項を棚卸しし、技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成などの候補を比較できる状態にする。そのうえで人間が MVP 構成を決め、`00-docs/decisions.md` に記録し、`architecture.md` / `technical_design.md` / `local_operations.md` へ反映する。そこまで通してから、`.ops`、`CLAUDE.md`、settings、rules / skills / agents、task packet 雛形を準備し、人間が安全に `proposed → ready → running → review → done / blocked` を判断できる状態を作る。
...
- 草案 → 00-docs → 未決定整理 → 候補比較 → 人間決定 → 設計ドキュメント化 → ハーネス設定 → 初回 triage → 最初の1タスク実行まで
...
- [ ] **仕様が曖昧なうちは実装タスクに入らない**
- [ ] **後続タスク全体に効く判断は Decision Gate を通す**。技術スタック・詳細設計・本番構成・認証情報管理などは実装タスクにしない
- [ ] **AIは候補整理・比較・影響分析まで**。最終決定は人間が行い、`decisions.md` に記録する
- [ ] **決定してから設計ドキュメントへ反映する**。`architecture.md` / `technical_design.md` / `local_operations.md` は人間決定の写しとして作る
...
草案があるプロジェクトをハーネスに載せるまでの流れは以下の主要ステップ。**順番に依存関係がある**ので飛ばさない。

| # | ステップ | なぜこの順番か |
|---:|---|---|
| 0 | 安全な作業場所を決める | 草案・設定・ハーネス関連ファイルを置く repo を決め、secrets や本番設定を最初から混ぜない |
| 1 | 草案を整理する | すべての下流ドキュメントの入力。ここが曖昧だと全部が曖昧になる |
| 2 | 00-docs 初期版を作る | 「計画の truth」の入口。ここでは決定 / 仮決定 / 未決定を分けるだけで、未決定を埋めない |
| 3 | 未決定事項を整理する | 何を人間が決める必要があるかを明文化する。実装タスク化はまだしない |
| 4 | セッションA2: 技術・構成候補を比較する | 技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成を options document にする |
| 5 | 人間ゲート: MVP構成を決める | **人間ゲート**。AIの比較表を材料に、人間が MVP の前提を選ぶ |
| 6 | セッションA3: 設計ドキュメントを作る | 決定内容を `decisions.md` に記録し、`architecture.md` / `technical_design.md` / `local_operations.md` へ反映する |
| 7 | .ops を作る | 「実行の truth」。queue / templates の器がないと task packet を置く場所がない |
| 8 | CLAUDE.md を作る | プロジェクトの憲法。00-docs と .ops の構造が決まってから書く |
| 9 | settings.json を作る | 強制力のある安全境界。方針(CLAUDE.md)を物理的に強制する |
| 10 | rules / skills / agents を作る | 定義(CLAUDE.md)と強制(settings)の間を埋める判断・手順・役割 |
| 11 | task packet 雛形を作る | triage の出力フォーマット。triage より先に必要 |
| 11.5 | スモークテストをする | ダミータスクで queue を1周させ、配管を本物のタスクより先に確認する |
| 12 | 初回 triage 前に未決定を分類する | 実装タスク化してよいもの、設計判断タスクに留めるもの、候補比較タスクにするものを分ける |
| 13 | 初回 triage をする | 00-docs と queue を読んで候補を作る。準備が揃って初めて意味を持つ |
| 14 | proposed を作る | triage の出力。AIが構造化・スコアリングする |
| 15 | 人間が ready に昇格する | **人間ゲート**。「今日何を回すか」は人間の判断 |
| 16 | worktree で実行する | 1 task = 1 worktree。人間が起動し、AIが自律実行する |
| 17 | review する | Review Agent が REVIEW.md を書く(別セッション・plan mode) |
| 18 | 人間が done / merge / blocked を判断する | **人間ゲート**。merge は最終承認 |

`.ops` の構造自体は先に作ってよい。ただし、本物の実装タスクを `running` に進める前に、最低限の MVP 構成決定と設計ドキュメント化を通す。未決定が多い場合でもハーネス導入は始められるが、未決定のまま危険な実装へは進まない。
...
## セッションA2: 未決定事項と MVP 構成候補の整理

| 項目 | 内容 |
|---|---|
| 読むファイル | 00-docs 初期5ファイル、草案(確認用) |
| 作るファイル | `00-docs/unresolved_items.md`, `00-docs/options/mvp_configuration_options.md`(必要なら `technology_stack_options.md`, `runtime_options.md`, `notification_options.md`, `credentials_management_options.md`, `local_operations_options.md` に分割) |
| 作らせてはいけないもの | 候補の採用決定、実装タスク、production deploy 前提、認証情報を読む実装、特定技術への決め打ち |
| 完了条件 | 技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成について、複数案・利点・欠点・影響範囲・未確認事項が整理されている |
| 分割する理由 | 未決定をすぐ実装タスクにすると、後続全体を巻き戻す。AIは候補整理・比較・影響分析に集中させ、人間が選べる材料を作る |

## 人間ゲート: MVP 構成の決定

| 項目 | 内容 |
|---|---|
| 人間が見るファイル | `00-docs/unresolved_items.md`, `00-docs/options/` 配下、必要なら草案 |
| 人間が決めること | MVP で採用する技術スタック、ローカル実行構成、通知方式、認証情報管理方針、ローカル運用方針。未決定のまま残すものも明示する |
| AIに頼んでよいこと | 選択肢の再比較、影響範囲の説明、決定案の文章化 |
| AIに頼んではいけないこと | 「おすすめで決めて」と丸投げすること、secrets や本番接続情報の作成・取得 |
| 完了条件 | 人間が選んだ内容だけが `00-docs/decisions.md` に DEC として記録され、未決定のまま残す項目も `unresolved_items.md` に残っている |

## セッションA3: 決定内容を設計ドキュメントへ反映

| 項目 | 内容 |
|---|---|
| 読むファイル | 00-docs 初期5ファイル、`unresolved_items.md`, `options/` 配下、更新済み `decisions.md` |
| 作るファイル | `00-docs/architecture.md`, `00-docs/technical_design.md`, `00-docs/local_operations.md`、必要に応じて `requirements.md` / `roadmap.md` / `status.md` の整合更新 |
| 作らせてはいけないもの | 実装コード、task packet、未決定項目の勝手な確定、本番構成の詳細化、認証情報そのもの |
| 完了条件 | 設計ドキュメントが `decisions.md` の決定と整合し、未決定項目は「未決定」として残っている。実装タスク化できる前提と、まだ設計判断が必要な前提が分かる |
| 分割する理由 | `architecture.md` / `technical_design.md` / `local_operations.md` は実装タスクの入力になる。人間決定の前に作ると、AIの仮置きが設計の正本に見えてしまう |
...
| 項目 | 内容 |
|---|---|
| 読むファイル | 設計書の §2(スコープ設計)・§7(Queue State Machine)、00-docs 初期5ファイル + A2/A3で作った設計関連ファイル、`.ops/` の実際の構造 |
...
## セッションF0: 初回 triage 前の未決定分類

| 項目 | 内容 |
|---|---|
| 読むファイル | 00-docs 全体、`decisions.md`, `unresolved_items.md`, `architecture.md`, `technical_design.md`, `local_operations.md`, `.ops/templates/task-packet/` |
| 作るファイル | `.ops/reports/triage/TRIAGE_PRECHECK_{date}.md`、必要なら `unresolved_items.md` の分類更新 |
| 作らせてはいけないもの | proposed task packet、ready 昇格、実装、worktree 作成、未決定事項の確定 |
| 完了条件 | 未決定事項が「実装タスク化してよい」「設計判断タスクにする」「候補比較タスクにする」「小さな検証タスクにする」「人間質問のまま残す」に分類されている |
| 分割する理由 | 初回 triage で未決定をまとめて task packet 化すると、実装してはいけないものまで proposed に混ざる。先に分類して、triage の入力を安全にする |

## セッションF: 初回 triage

| 項目 | 内容 |
|---|---|
| 読むファイル | CLAUDE.md、00-docs 全部、`TRIAGE_PRECHECK_{date}.md`、`.ops/queue/`、`.ops/templates/task-packet/` |
...
| 完了条件 | proposed に 3〜5件の自己完結した task packet があり、各 BRIEF.md を人間が読んで意味が分かる。仕様・技術・構成が未決定の項目は、実装ではなく設計判断タスク / 候補比較タスク / 小さな検証タスク / spec-question になっている |
...
5. `00-docs/status.md`
6. `00-docs/unresolved_items.md`
7. `00-docs/options/mvp_configuration_options.md`(必要なら分割)
8. 人間が決めた内容を `00-docs/decisions.md` に追記
9. `00-docs/architecture.md`
10. `00-docs/technical_design.md`
11. `00-docs/local_operations.md`
12. `.ops/` ディレクトリ構造(`queue/` 7状態, `runs/`, `reports/`, `config/`)
13. `.ops/DASHBOARD.md`(空テンプレート)
14. `.ops/config/parallel.yaml`
15. `.ops/templates/task-packet/`
16. `CLAUDE.md`
17. `.claude/settings.json`(+ `settings.local.json`)
18. `.claude/rules/`
19. `.claude/agents/`
20. `.claude/skills/`
...
- `00-docs/unresolved_items.md`, `00-docs/options/`(後続全体の判断材料になるため、候補と影響範囲を丁寧に分ける)
- `00-docs/architecture.md`, `technical_design.md`, `local_operations.md`(人間が決めた MVP 構成を写す。未決定のまま決め打ちしない)
...
- 本番向けの `deployment.md` / `production_operations.md` — MVP の本番構成を人間が決めてから
...
| | |
|---|---|
| 書くこと | requirements / roadmap / decisions候補から集めた未決定事項。各項目に「何を決める必要があるか」「影響範囲」「実装前に必須か」「候補比較が必要か」「人間への質問」を書く |
| 書かないこと | AIによる採用決定、実装手順、secrets や本番接続情報 |

```text
作成プロンプト例:
00-docs の初期5ファイルを読み、00-docs/unresolved_items.md を作成してください。
未決定事項を「仕様」「技術スタック」「実行構成」「通知方式」「認証情報管理」
「ローカル運用」「本番構成」「その他」に分類してください。
各項目に、決めないと何が進められないか、実装前に必須か、候補比較が必要か、
人間に聞くべき質問を1〜3行で書いてください。
未決定事項を勝手に決めないでください。
```
...
| | |
|---|---|
| 書くこと | 技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成について、複数案、利点、欠点、影響範囲、リスク、未確認事項、MVPでの扱いやすさ |
| 書かないこと | 最終決定、採用済みの断定、認証情報そのもの、本番構成の詳細手順 |

```text
作成プロンプト例:
00-docs/unresolved_items.md と初期5ファイルを読み、
00-docs/options/mvp_configuration_options.md を作成してください。
対象は 技術スタック / 実行構成 / 通知方式 / 認証情報管理 / ローカル運用構成。
各テーマについて2〜4案を出し、利点・欠点・影響範囲・未確認事項・MVPでの扱いやすさを
表で比較してください。
最後に「AIとしての推奨」ではなく「人間が選ぶ時の判断軸」を書いてください。
採用決定はしないでください。
```

## architecture.md / technical_design.md / local_operations.md

| | |
|---|---|
| 書くこと | 人間が `decisions.md` に記録した MVP 構成に基づく設計。architecture は全体構成、technical_design は実装上の分割・データ・主要インターフェース、local_operations はローカル起動・検証・ログ確認・停止手順 |
| 書かないこと | 人間が決めていない技術の採用、詳細な本番構成、secrets の値、未検証の外部サービス前提 |

```text
作成プロンプト例:
00-docs/decisions.md、unresolved_items.md、options/ 配下を読み、
人間が決定済みの内容だけを使って architecture.md / technical_design.md /
local_operations.md を作成してください。
未決定の項目は「未決定」と明記し、実装タスクの前提にしないでください。
decisions.md にない決定を新しく作らないでください。
```
...
- Decision Gate の定義(後続全体に効く技術・構成判断は options → 人間決定 → decisions.md → 設計ドキュメント → task packet の順)
- Done / Review の定義
...
および 00-docs/ の初期5ファイル、unresolved_items.md、architecture.md、
technical_design.md、local_operations.md を読み、このプロジェクトのルート CLAUDE.md を作成してください。
...
## 実装 task packet にしてはいけない状態

1つでも当てはまるなら、まだ実装タスク化しない(00-docs の「未決定」か inbox に戻すか、設計判断タスク / 候補比較タスク / 小さな検証タスクに変換する):
...
- 実行構成・通知方式・認証情報管理・ローカル運用構成が未決定なのに、それを前提とする
- 後続タスク全体に影響する判断を含んでいる(まず options document と人間決定が必要)
- 人間がまだ仕様判断していない
- secrets / 認証情報が必要
- 本番環境に触る
- 課金が発生しうる
...
- 設計判断タスク(「認証情報管理方針の選択肢を整理し、人間が決める論点をまとめる」)
- 候補比較タスク(「データ保存方式の候補とトレードオフをまとめる」)
- 小さい検証タスク(「雛形が design D の必須ファイル定義を満たすか検査する」)
- 小さい技術検証タスク(「候補Aがローカルで起動可能か、ダミーデータだけで確認する」。secrets / 本番 / 課金なしに限る)
...
- [ ] 実装タスクの場合、前提となる仕様・技術・構成判断が `decisions.md` と設計ドキュメントに記録済み
- [ ] 1〜2時間以内で終わる見込み
...
- 設計候補比較(技術スタック、実行構成、通知方式、認証情報管理、ローカル運用構成)
- 未決定事項整理(実装前に必須 / 後回し可 / 人間質問 / 検証で潰せる、に分ける)
- CLAUDE.md の整合チェック
- task packet 雛形や `blocked_reason.md` / `REVIEW.md` テンプレートの改善
- DASHBOARD.md の初期化・生成確認
- 小さい調査タスク(読み取りのみで完結するもの)
- 小さな検証タスク(secrets・本番・課金なしで、候補の成立性だけを見るもの)

## 初回に向いていないタスク

- 本格実装(複数機能・複数ディレクトリにまたがる変更)
- 技術選定未決定のままの実装
- 実行構成・通知方式・認証情報管理が未決定のままの実装
- 認証情報を扱う実装
- 本番構成の設計・構築・デプロイ
- 外部サービス連携
- 課金が絡む操作
...
各ステップは §2 のセッションA〜Gに対応する。**1日に全部やろうとしない**(目安は §12 の1週間プラン)。
...
### Step 2.5: 未決定事項と候補を整理する(セッションA2)

| | |
|---|---|
| 人間が見るファイル | `unresolved_items.md`, `options/mvp_configuration_options.md` |
| AIに頼むこと | §4 のプロンプトで未決定事項を分類し、技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成の候補を比較させる |
| 次に進む条件 | 各候補の利点・欠点・影響範囲・未確認事項が見え、人間が選ぶための判断軸が揃っている |
| やってはいけないこと | AIに候補を採用させる。候補比較を実装タスクに混ぜる |

### Step 2.6: MVP 構成を決める(人間ゲート)

| | |
|---|---|
| 人間が見るファイル | `unresolved_items.md`, `options/` 配下、草案 |
| AIに頼むこと | 不明点の再説明、比較表の補足、決定候補の文章化 |
| 次に進む条件 | 人間が決めた項目だけが `decisions.md` に DEC として記録され、未決定のまま残す項目も明記された |
| やってはいけないこと | 「AIのおすすめ」をそのまま決定扱いにする。本番構成や認証情報の実体をこの場で作らせる |

### Step 2.7: 設計ドキュメントを作る(セッションA3)

| | |
|---|---|
| 人間が見るファイル | `architecture.md`, `technical_design.md`, `local_operations.md` |
| AIに頼むこと | `decisions.md` に記録済みの内容だけを、設計ドキュメントへ反映させる |
| 次に進む条件 | 設計ドキュメントが decisions と整合し、未決定項目は未決定として残っている |
| やってはいけないこと | decisions.md にない判断を設計ドキュメントで確定させる。実装タスクを作る |
...
無害なダミータスクで proposed → done まで動かし、配管(packet 移動、HANDOFF 更新、DASHBOARD 反映)を確認する。**ここを飛ばして本物のタスクに進まない**。MVP 構成の Decision Gate が未通過の場合も、本物の実装タスクには進まない。

### Step 6.7: 初回 triage 前に未決定を分類する(セッションF0)

| | |
|---|---|
| 人間が見るファイル | `TRIAGE_PRECHECK_{date}.md`, `unresolved_items.md` |
| AIに頼むこと | 未決定事項を「実装タスク化してよい」「設計判断タスク」「候補比較タスク」「小さな検証タスク」「人間質問」に分類させる |
| 次に進む条件 | 実装候補と判断候補が混ざらず、初回 triage で作ってよい proposed の種類が明確になった |
| やってはいけないこと | この分類なしに、未決定事項をまとめて proposed task packet にする |
...
| AIに頼むこと | 「triage して」(§11 のプロンプト)。F0 の分類に従い、実装タスク・設計判断タスク・候補比較タスクを混ぜてよいが、未決定前提の実装は作らせない |
...
### 11-1a. 未決定事項と候補を整理する(セッションA2)

- 使う場面: 00-docs 初期5ファイルを人間が確認した後 / 期待する出力: `unresolved_items.md` と options document
- 注意点: 候補整理で止める。採用決定はしない

```text
00-docs の初期5ファイルを読み、未決定事項を整理してください。
出力:
1. 00-docs/unresolved_items.md
2. 00-docs/options/mvp_configuration_options.md
...
- 認証情報管理
- ローカル運用構成
- 本番構成(この時点では詳細化せず、未決定/後回しの扱いを明記)

各候補について、利点・欠点・影響範囲・未確認事項・MVPでの扱いやすさを比較してください。
最終決定はしないでください。最後に、人間が決めるべき項目をチェックリストにしてください。
```

### 11-1b. MVP 構成の決定を記録する(人間ゲート後)

- 使う場面: 人間が options を読んで採用方針を決めた後 / 期待する出力: `decisions.md` への DEC 追記
- 注意点: 人間が明示した内容だけを記録する

```text
以下は私が決めた MVP 構成です:
{決定内容を箇条書きで書く}

この内容だけを 00-docs/decisions.md に DEC として記録してください。
理由と影響範囲も、私が書いた内容と options document の比較材料から要約してください。
私が明示していない項目は決定扱いにせず、unresolved_items.md に残してください。
実装タスクは作らないでください。
```

### 11-1c. 設計ドキュメントを作る(セッションA3)

- 使う場面: MVP 構成が `decisions.md` に記録された後 / 期待する出力: architecture / technical design / local operations
- 注意点: decisions.md にない決定を作らない

```text
00-docs/decisions.md、unresolved_items.md、options/ 配下を読み、
決定済みの内容だけを使って以下を作成してください。
- 00-docs/architecture.md
- 00-docs/technical_design.md
- 00-docs/local_operations.md

未決定の項目は「未決定」と明記し、実装タスクの前提にしないでください。
本番構成・認証情報・外部サービス接続は、決定済みでない限り詳細化しないでください。
必要なら requirements.md / roadmap.md / status.md の整合更新案も出してください。
```
...
### 11-8a. 初回 triage 前に未決定を分類する(セッションF0)

- 使う場面: スモークテスト後、本物の proposed を作る前 / 期待する出力: TRIAGE_PRECHECK
- 注意点: task packet はまだ作らない

```text
初回 triage の前段として、未決定事項を分類してください。
読むもの:
- 00-docs/ 全体
- 00-docs/decisions.md
- 00-docs/unresolved_items.md
- 00-docs/architecture.md
- 00-docs/technical_design.md
- 00-docs/local_operations.md
...
出力:
- .ops/reports/triage/TRIAGE_PRECHECK_{date}.md

分類:
1. 実装タスク化してよいもの
2. まだ設計判断タスクにすべきもの
3. 候補比較タスクにすべきもの
4. 小さな検証タスクにできるもの(secrets・本番・課金なし)
5. 人間質問のまま残すもの

この段階では proposed task packet を作らないでください。
未決定事項を勝手に決めないでください。
```

### 11-9. 初回 triage をする(セッションF)

- 使う場面: F0 の分類後の最初の候補出し / 読ませるファイル: CLAUDE.md, 00-docs, TRIAGE_PRECHECK, queue / 期待する出力: TRIAGE_REPORT, CANDIDATES, proposed 3〜5件
- 注意点: ready 昇格はさせない

```text
triage してください。
00-docs、.ops/reports/triage/TRIAGE_PRECHECK_{date}.md、.ops/queue/ の状態を読み、
TRIAGE_REPORT と CANDIDATES を生成し、
proposed/ に task packet を作成してください。
ルール:
- 候補は最大5件まで
- [未決定] の仕様・技術・構成に依存する実装タスクは作らない
- 未決定事項は、F0 の分類に従って設計判断タスク / 候補比較タスク /
  小さな検証タスク / spec-question にする
- 技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成が
  未決定のまま、それを前提とする実装タスクは作らない
...
### Day 3: 未決定整理と候補比較(セッションA2)

| | |
|---|---|
| 目的 | 未決定事項を実装前に見える化し、MVP 構成の候補を比較する |
| 人間が見るファイル | `unresolved_items.md`, `options/mvp_configuration_options.md` |
| AIに頼むこと | プロンプト 11-1a |
| 完了条件 | 技術スタック・実行構成・通知方式・認証情報管理・ローカル運用構成の候補と判断軸が揃っている |
| やってはいけないこと | AIに採用決定までさせる。候補比較からそのまま実装タスクを作る |

### Day 4: MVP 構成決定 + 設計ドキュメント化(人間ゲート → セッションA3)

| | |
|---|---|
| 目的 | 人間が MVP 構成を決め、実装タスクの入力になる設計ドキュメントを作る |
| 人間が見るファイル | options document、`decisions.md`, `architecture.md`, `technical_design.md`, `local_operations.md` |
| AIに頼むこと | 決めた内容をプロンプト 11-1b で記録 → 別セッションで 11-1c |
| 完了条件 | decisions と設計ドキュメントが整合し、未決定項目が残る場合は明示されている |
| やってはいけないこと | 本番構成や認証情報をこの場で詳細化する。未決定を設計文書で決定済みにする |

### Day 5: .ops / CLAUDE.md / settings / rules 初期作成(セッションB → C → D → E1)

| | |
|---|---|
| 目的 | 実行の器、憲法、強制力のある安全境界、最小ルールを作る |
| 人間が見るファイル | .ops のツリー、CLAUDE.md、settings.json、safety.md |
| AIに頼むこと | プロンプト 11-7 → 11-8 → 別セッションで 11-2 → 11-3 → 11-4 |
| 完了条件 | .ops 構造が設計書と一致、CLAUDE.md のチェックリスト通過、起動テストと secrets deny の実地テスト通過 |
| やってはいけないこと | 構造作業と憲法執筆を1セッションで混ぜる。deny テストを省略する。便利さのために allow を増やす |

### Day 6: agents / skills 作成 + スモークテスト + F0分類(セッションE2 → E3 → B2 → F0)

| | |
|---|---|
| 目的 | 役割と手順を整え、ダミータスクで配管を確認し、本物の triage 前に未決定を分類する |
| 人間が見るファイル | agents 3ファイル、ダミータスクの動き(DASHBOARD)、`TRIAGE_PRECHECK_{date}.md` |
| AIに頼むこと | プロンプト 11-5 → 11-6 → ダミータスク1周 → 11-8a |
| 完了条件 | ダミータスクが proposed → done まで1周し、未決定事項が実装 / 設計判断 / 候補比較 / 検証 / 人間質問に分類された |
| やってはいけないこと | スモークテストや F0 分類を飛ばして本物の proposed を作る |

### Day 7: 初回 triage と最初の安全なタスク(セッションF → G)

| | |
|---|---|
| 目的 | 本物の候補を作り、人間が中身を確認し、最初の1タスクだけ実行する |
| 人間が見るファイル | TRIAGE_REPORT、proposed 各件の BRIEF.md |
| AIに頼むこと | プロンプト 11-9 → 11-10 |
| 完了条件 | proposed 3〜5件を全件確認し、ready 候補を1本だけ決めた。最初の1本は設計候補比較・未決定整理・小さな検証タスクでもよい |
| やってはいけないこと | 候補を確認せずに昇格する。技術・構成未決定のまま実装へ進める。並列で2本目を始める |

時間が残る場合だけ、プロンプト 11-11 → 11-12 →(別セッションで)11-15 に進む。完了条件は review まで到達し、人間が merge / 差し戻し / 保留のいずれかを**判断した**こと。merge できなくてもサイクル完走が成果。

## 1週間で終わらない場合の延長方針

- **順番を守ったまま日数を伸ばす**。工程を飛ばして縮めない
- 詰まりやすいのは Day 2(未決定が多い)、Day 3〜4(候補比較と人間決定)、Day 5(settings の実仕様確認)。ここは2日以上かけてよい
- 未決定が多すぎて triage で実装候補がゼロになる場合、それは失敗ではない。**「設計候補比較」「未決定を潰す整理タスク」「小さな検証タスク」を最初の数本にする**のが正しい進行
- 延長中も Day 6 のスモークテストと F0 分類だけは省略しない(飛ばすと、最初の本番タスクで配管の不具合と設計未決定の問題が混ざる)
...
- [ ] `00-docs/unresolved_items.md` と `00-docs/options/` がある
- [ ] MVP 構成について、人間が決めた内容だけが `00-docs/decisions.md` に記録されている
- [ ] `00-docs/architecture.md`, `technical_design.md`, `local_operations.md` が decisions と整合している
- [ ] `CLAUDE.md` がある
...
- [ ] 初回 triage 前の `TRIAGE_PRECHECK_{date}.md` がある
...
- [ ] 実装タスクの場合、前提となる設計判断が `decisions.md` と設計ドキュメントに反映済み
- [ ] afk_safe か判断済み
- [ ] 依存する running タスクがない
...
- [ ] worktree / branch の削除
- [ ] 技術スタックや仕様の確定
- [ ] MVP 構成(実行構成・通知方式・認証情報管理・ローカル運用方針)の確定
